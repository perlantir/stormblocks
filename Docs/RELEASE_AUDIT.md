...
| Performance optimization | Primitive pooling, Low Detail fallback, URP mobile settings, optimized design-source imports, scene-budget guard, and modern-iPhone Game Performance/Power traces exist; latest full-detail budget is 448 renderers and 163,432 triangles. Latest 60.86-second Power trace on iPhone 17 Pro Max reports nominal thermal state, average GPU Active Time `0.549 ms`, and average Frame Interval `33.620 ms`; older-device and longer interactive profiling remain pending. | Partial physical pass; older-device profiling pending |
| Physical QA handoff | `Docs/PHYSICAL_QA_RUNBOOK.md`, `Docs/FIVE_RUN_QA_SCORECARD.md`, `Scripts/verify_five_run_scorecard.sh`, and `Scripts/device_qa_session.sh` define launch, structured five-run QA, Game Center, TestFlight, and profiling steps. The release audit now fails if the physical QA checklist item is checked without a passing filled scorecard. | Local pass; physical execution pending |
| GitHub static verification | `.github/workflows/release-static.yml` runs the same static release checks on pushed branch heads. The current branch-head run is verified in GitHub Actions after each push and reported in the release handoff instead of being treated as a static local artifact. | Local pass; remote run verified after push |
...
| Physical-device install | Current-source `/tmp/stormblocks-device-install.json` reports `"outcome" : "success"` for paired iPhone `907E2EE7-9C7B-5D0D-9EC0-32E69912287D`; latest retry ran at `2026-05-05 17:56:08Z`. | Pass |
...
Storm Blocks is locally release-candidate-ready through tests, signed build, current-source physical install/launch, archive, and App Store Connect IPA export. It is not final-release complete because the App Store Connect app record, live Game Center configuration, TestFlight validation, and physical QA/performance gates are still open.
