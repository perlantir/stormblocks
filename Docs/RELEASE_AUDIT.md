...
| QA tests | Latest EditMode: 25/25 at `2026-05-05 15:09:10Z`; latest PlayMode: 7/7 at `2026-05-05 15:09:18Z`. | Pass |
...
| Performance optimization | Primitive pooling, Low Detail fallback, URP mobile settings, optimized design-source imports, and scene-budget guard exist; latest full-detail budget is 356 renderers and 133,932 triangles. | Local pass; physical profiling pending |
...
| TestFlight upload | Current-source upload probe authenticated to App Store Connect at `2026-05-05 15:16:59Z`; Apple returned HTTP 200 with `data: []` for `filter[bundleId]=com.perlantir.stormblocks`, then Xcode failed at `IDEDistributionFetchAppRecordStep` with `missingApp(bundleId: "com.perlantir.stormblocks")`. | Blocked by missing app record |
