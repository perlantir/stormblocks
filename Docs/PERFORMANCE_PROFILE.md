...
- `STORMBLOCKS_PROFILE_TIME=2m Scripts/device_qa_session.sh profile-game` captured `StormBlocksUnity/Builds/DeviceProfiles/stormblocks-game-performance-20260505T180036Z.trace` on the same iPhone; the trace is 104 MB and viewable, but the target app exited after 10 seconds so it is launch/runtime evidence, not a full interactive profile.
- `STORMBLOCKS_PROFILE_TIME=1m Scripts/device_qa_session.sh profile-game` retried against the already-running app and captured `StormBlocksUnity/Builds/DeviceProfiles/stormblocks-game-performance-20260505T181049Z.trace`; the trace is 100 MB, `xcrun xctrace export --toc` succeeds, reports target `iPhone 17 Pro Max`, iOS `26.3 (23D127)`, process `StormBlocks`, `end-reason` = `Time limit reached`, and a 10-second windowed Game Performance duration. A post-trace process check still listed `StormBlocks` running as process id `1589`.
- `STORMBLOCKS_PROFILE_TIME=1m Scripts/device_qa_session.sh profile-power` captured `StormBlocksUnity/Builds/DeviceProfiles/stormblocks-power-20260505T180251Z.trace` on the same iPhone and reached the time limit.
- Exported latest Power Profiler metrics report target `iPhone 17 Pro Max`, iOS `26.3 (23D127)`, process `StormBlocks`, nominal thermal state for 60.86 seconds, GPU Active Time average `0.549 ms` across 61 sampled windows / 1,819 frames, and Frame Interval average `33.620 ms` across the same sample. A post-trace `devicectl device info processes` check still listed `StormBlocks` running as process id `1589`.
